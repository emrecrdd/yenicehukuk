...
const EventEdit = () => {
  ...
  useEffect(() => {
    if (eventData?.data?.data) {
      ...
      setFormData({
        ...
        hearing_type: event.hearing_type || 'other',
        last_hearing_result: event.last_hearing_result || '',
        opposing_counsel: event.opposing_counsel || '',
        expense_status: event.expense_status || 'pending',
  ...
  const mutation = useMutation({
    ...
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Duruşma güncellenemedi');
    },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Duruşma başarıyla silindi');
      navigate('/calendar');
    },
    onError: (error) => {
      toast.error(error.response?.data?.message || 'Duruşma silinemedi');
    },
  });
  ...
  const handleDelete = () => {
    if (window.confirm('Bu duruşmayı silmek istediğinize emin misiniz?')) {
