...
export default PowerOfAttorneyList; import { useState } from 'react';
import { Link } from 'react-router-dom';
import { useQuery } from '@tanstack/react-query';
import { powerOfAttorneyApi } from '../../features/power-of-attorney/powerOfAttorney.api.js';
          ...
          <Table>
            <Table.Head>
              <Table.Row>
                ...
                <Table.HeadCell>Bitiş</Table.HeadCell>
                <Table.HeadCell>Durum</Table.HeadCell>
                <Table.HeadCell>Belge</Table.HeadCell>  {/* ✅ YENİ SÜTUN */}
                <Table.HeadCell>İşlem</Table.HeadCell>
              </Table.Row>
            </Table.Head>
            <Table.Body>
                      ...
                      {item.documents && item.documents.length > 0 ? (
                        <div className="flex items-center gap-1">
                          <span className="text-sm text-gray-500">
                            {getFileIcon(item.documents[0].file_type)}
                          </span>
                          <button
                            onClick={() => handleDownload(item.documents[0].id, item.documents[0].original_name || item.documents[0].name)}
                          ...
                          {item.documents.length > 1 && (
                            <span className="text-xs text-gray-400">+{item.documents.length - 1}</span>
                          )}
                        </div>
                      ) : (
